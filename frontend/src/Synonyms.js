...
function lookForWords(text, lan) {
    ...
    let lang = lan+'-ru'
    let response = 'https://dictionary.yandex.net/api/v1/dicservice.json/lookup?key='+yandex_dictionary_key+'&lang='+lang+'&text='+text;
...
export default function monitorSynonyms(id, lang){
    document.onmouseup = document.onkeyup = document.onselectionchange = function() {
        document.getElementById(id).value = getSelectionText();
        let output = getSelectionText();
        let syn = lookForWords(output, lang);
        console.log(syn);
        // let el = document.createElement("div");
        // el.innerHTML = syn.toString();
        setTimeout(2000);
